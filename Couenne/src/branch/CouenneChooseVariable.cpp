...
void CouenneChooseVariable::registerOptions (Ipopt::SmartPtr <Bonmin::RegisteredOptions> roptions) {
  ...
  roptions -> AddLowerBoundedIntegerOption
    ("cont_var_priority",
     ...
     "Higher values mean smaller priority, so if this parameter is set to 1001 or higher, if a branch-and-bound node has at least one integer" 
  ...
  roptions -> AddLowerBoundedIntegerOption
    ("int_var_priority",
     ...
     1, 1000,
     "When branching, this is compared to the priority of continuous variables, whose priority is fixed to 2000, and SOS, whose priority is 10. "
     "Higher values mean smaller priority, so if this parameter is set to 1001 or higher, if a branch-and-bound node has at least one" 
     " integer variable whose value is fractional, then branching will be performed on that variable."
    );
